...
Why labour this? Because the level of a variable silently decides which statistics and which charts are honest. You can take the **mean** of a ratio variable but not of a nominal one. You compare categories with a **bar chart** and show the distribution of a numeric variable with a **histogram** — and mixing them up is a common way to draw a misleading picture. A number stored in your data isn't automatically a *quantity*: a postal code and a phone number are digits that you must treat as labels, because averaging them is meaningless.
